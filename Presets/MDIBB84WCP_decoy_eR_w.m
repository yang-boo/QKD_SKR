...
function [protocolDescription,channelModel,leakageEC,parameters,solverOptions] = MDIBB84WCP_decoy_eR_w()
...
function [protocolDescription,channelModel,leakageEC]=setDescription()
    ...
    channelModel=str2func('MDIBB84WCPChannel_eR_w');
    leakageEC=str2func('generalEC_MDI');
...
%     parameters.scan.thetaA = 0:pi/64:pi/4; %misalignment angle between Alice-Charlie
    parameters.scan.eta = 10.^(-0.2*([0:10:80,81:1:100])/10); %channel transmittance between Alice-Charlie (half of total distance)
    ...
    parameters.fixed.thetaA = 0.091; %misalignment angle between Alice-Charlie sin^2(theta) 
    parameters.fixed.thetaB = -0.091; %misalignment angle between Bob-Charlie  sin^2(theta)
