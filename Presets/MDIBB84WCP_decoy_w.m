...
function [protocolDescription,channelModel,leakageEC,parameters,solverOptions] = MDIBB84WCP_decoy_w()
...
function [protocolDescription,channelModel,leakageEC]=setDescription()
    ...
    channelModel=str2func('MDIBB84WCPChannel_w');
...
%     parameters.scan.thetaA = 0:pi/64:pi/4; %misalignment angle between Alice-Charlie
    parameters.scan.eta = 10.^(-0.2*([0:10:90,91:1:110])/10); %channel transmittance between Alice-Charlie (half of total distance)
    ...
    parameters.fixed.thetaA = 0.091; %misalignment angle between Alice-Charlie sin^2(theta) 0.091
