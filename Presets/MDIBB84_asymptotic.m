...
function [protocolDescription,channelModel,leakageEC,parameters,solverOptions] = MDIBB84_asymptotic()
...
%     parameters.scan.thetaA = 0 %0:pi/32:pi/2; %misalignment angle between Alice-Charlie
    parameters.scan.eta = 10.^(-0.2*((0:12:0)/2)/10); %channel transmittance between Alice-Charlie (half of total distance)
...
%     parameters.fixed.eta = 1; %channel transmittance between Alice-Charlie (half of total distance)
    parameters.fixed.thetaA = 0.0915; %misalignment angle between Alice-Charlie
    parameters.fixed.thetaB = -0.0915; %misalignment angle between Bob-Charlie
