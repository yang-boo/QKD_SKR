...
function [protocolDescription,channelModel,leakageEC,parameters,solverOptions] = MDIBB84WCP_decoy()
...
function [protocolDescription,channelModel,leakageEC]=setDescription()
    ...
    channelModel=str2func('MDIBB84WCPChannel');
...
%     parameters.scan.thetaA = 0:pi/64:pi/4; %misalignment angle between Alice-Charlie
    parameters.scan.eta = flip(10.^(-0.2*(0:26:0)/10)); %channel transmittance between Alice-Charlie (half of total distance)

    %%%%%%%%%%%%%%%% 2.fixed parameters %%%%%%%%%%%%%%%%
    %optional; the constant values can be either numerical or array/matrices
    parameters.fixed.pz = 0.8; %basis choice probability (for Z basis); only Z basis is used for coding
    parameters.fixed.pd = 1e-6; %dark count probability
    parameters.fixed.f = 1; %error-correction efficiency
    parameters.fixed.thetaA = 0.0915; %misalignment angle between Alice-Charlie sin^2(theta) 0.0915
    parameters.fixed.thetaB = -0.0915; %misalignment angle between Bob-Charlie  sin^2(theta)
    parameters.fixed.mu1 = 0.4; %first intensity (signal)
    parameters.fixed.mu2 = 0.02; %second intensity (decoy 1)
