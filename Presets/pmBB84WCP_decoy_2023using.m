...
function [protocolDescription,channelModel,leakageEC,parameters,solverOptions] = pmBB84WCP_decoy_2023using()
...
function [protocolDescription,channelModel,leakageEC]=setDescription()
    ...
    protocolDescription=str2func('pmBB84LossyDescription_2023using');
    channelModel=str2func('pmBB84WCPChannel_2023using'); %contains asymptotic decoy state analysis in file
...
function parameters=setParameters()
    ...
    parameters.scan.eta = 10.^(-0.2*(0:30:0)/10); %channel transmittance

    %%%%%%%%%%%%%%%% 2.fixed parameters %%%%%%%%%%%%%%%%
    %optional; the constant values can be either numerical or array/matrices
    
    parameters.fixed.ed = 0.033; %misalignment, defined as single-photon error, i.e. sin^2(theta) % 光路以及探测器等带来的系统误差，一般参考为 1.5% 或 3.3%
    parameters.fixed.pz = 0.5; %basis choice probability (for Z basis)
    parameters.fixed.pd = 1e-5; %1e-6; %dark count probability
    parameters.fixed.f = 1; %error correction efficiency
    parameters.fixed.etad = 0.2; %0.045; %detector efficiency % 探测器的单光子探测效率，APD类的一般为10%～15%，超导类的可达50%
...
function solverOptions=setOptions()
    ...
    solverOptions.optimizer.linearResolution = 5; %resolution in each dimension (for brute force search and coordinate descent)
    ...
    solverOptions.solver1.initmethod = 2; %minimizes norm(rho0-rho) or -lambda_min(rho), use method 1 for finite size, 2 for asymptotic v1
