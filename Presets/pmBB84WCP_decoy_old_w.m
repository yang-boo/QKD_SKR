...
function [protocolDescription,channelModel,leakageEC,parameters,solverOptions] = pmBB84WCP_decoy_old_w()
...
function [protocolDescription,channelModel,leakageEC]=setDescription()
    ...
    protocolDescription=str2func('pmBB84LossyDescription_w');
    channelModel=str2func('pmBB84WCPChannel_old_w'); %contains asymptotic decoy state analysis in file
    leakageEC=str2func('generalEC_BB84');
...
function parameters=setParameters()
    ...
    parameters.scan.eta = 10.^(-0.2*([0:10:110,111:1:140])/10); %channel transmittance

    %%%%%%%%%%%%%%%% 2.fixed parameters %%%%%%%%%%%%%%%%
    %optional; the constant values can be either numerical or array/matrices
    
    parameters.fixed.ed = 0.033; %misalignment, defined as single-photon error, i.e. sin^2(theta)
    ...
    parameters.fixed.etad = 0.2; %0.045; %detector efficiency
