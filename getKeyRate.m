...
% 计算关键汇率的核心后台函数。
% 该函数接收协议描述、信道模型（期望值）和纠错泄漏，并返回上界/下界和间隙。
% 它调用求解器 1 和求解器 2 并返回密钥率。
...
% 输入数据结构：
% 协议描述： [keyMap,krausOperators,observables,obsMask(可选) 
% 通道模型： [期望值、expMask（可选）probDist/errorRate、pSift］
% EC 描述： [leakageEC］
% 求解器选项： [全局设置、求解器 1、求解器 2] 
% (可选)参数和名称列表：可使用 findParameter("PARAMETER_NAME",solverOptions) 进行检索
...
%% FUNCTION NAME: getKeyRate
% 计算密钥率的核心后台函数。该函数接收
% 协议描述、信道模型（期望值）和纠错泄漏，并返回上下限和间隙。
% 调用求解器 1 和求解器 2 并返回密钥率。
%
% 输入数据结构：
% 协议描述： [keyMap,krausOperators,observables,obsMask(optional)] 
% 通道模型：[expectations,expMask(optional)probDist/errorRate,pSift］
% EC 描述： [leakageEC］
% 求解器选项： [globalSetting, solver1, solver2] 
% (可选）参数和名称列表：可使用 findParameter("PARAMETER_NAME",solverOptions)获取。
%
% 输出数据结构：
% lowerBound, upperBound, FWBound, success flag
%%
function [lowerBound, upperBound, FWBound, debugInfo]=getKeyRate(protocolDescription,channelModel,leakageEC,solverOptions,p,names)
    ...
    try
        ...
        elseif(strcmp(solverOptions.solver1.name,'finite'))
            ...
            d=findParameter("alphabet",names,p); %the encoding alphabet size 编码字母表大小
            n = N*(1-ptest)*sum(channelModel.pSift); %received coding signals 收到的编码信号
