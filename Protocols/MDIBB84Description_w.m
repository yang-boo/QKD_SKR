...
function protocolDescription = MDIBB84Description_w(names,p)
...
%     varNames=["eta","pz","pd","f","thetaA","thetaB","mu1","mu2","mu3","etad","q"];
    ...
    krausOpZ0 = kron(kron(kron(kron([1;0], diag([1,0,0,0])), diag([1,0,0,0])) + kron(kron([0;1], diag([0,1,0,0])), diag([0,1,0,0])), diag([1,1,0])),[1;0]);
    krausOpZ1 = kron(kron(kron(kron([1;0], diag([1,0,0,0])), diag([0,1,0,0])) + kron(kron([0;1], diag([0,1,0,0])), diag([1,0,0,0])), diag([1,1,0])),[0;1]);
    krausOp={krausOpZ0,krausOpZ1};
